...
from ..utils.io import (
    ...
    list_remote_interface_logs,
    read_jsonl,
    write_json,
)


def stable_interface_run_dir(archive_root: str, date_str: str, patch_id: str, patch_set: str) -> str:
    """生成接口测试运行目录路径"""
    return os.path.join(
        archive_root,
        date_str,
        f"interface_p{patch_id}_ps{patch_set}",
    )
...
def iter_interface_day_strings(base_url: str, days: int) -> Iterator[tuple[str, str]]:
    """迭代接口测试的日期目录，返回(原始日期字符串, 标准化日期字符串)"""
...
def crawl_interface_test_incremental(base_url: str, archive_root: str, days: int = 7) -> list[str]:
    """增量下载新的接口测试日志文件，并返回新创建的运行目录列表。"""
    new_runs: list[str] = []

    for raw_date_str, normalized_date_str in iter_interface_day_strings(base_url, days):
        day_url = base_url.rstrip("/") + f"/{raw_date_str}/"
        logs = list_remote_interface_logs(day_url)
        if not logs:
            continue
        ...
        for item in logs:
            run_dir = stable_interface_run_dir(
            ...
            meta = {
                ...
                "test_type": "interface_test",  # 区分测试类型
            }
